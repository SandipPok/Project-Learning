<template>
   <div class="min-h-screen bg-gray-50">
          ...
          <div class="p-2">
            <div v-for="category in categories" :key="category.id" class="mb-2">
              ...
              <div v-if="expandedCategories[category.name]">
                <button
                  v-for="topic in topicsByCategory[category.name]" :key="topic.id"
                  @click="handleTopicSelect(topic)"
                  :class="[
                    'w-full text-left px-6 py-2 text-sm rounded-md transition-colors',
                    selectedTopic?.id === topic.id ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700 hover:bg-gray-50'
                  ]"
                >
                  {{ topic.title }}
                </button>
        ...
        <div class="col-span-9 space-y-6">
          <div v-if="selectedTopic" class="bg-white rounded-lg shadow-sm">
            ...
            <div class="p-4 grid grid-cols-2 gap-4">
              <div
                v-for="subtopic in selectedTopic.subtopics" :key="subtopic.id"
                @click="handleSubtopicSelect(subtopic)"
                :class="[
                  'p-4 rounded-lg border-2 transition-colors cursor-pointer',
                  selectedSubtopic?.id === subtopic.id ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 hover:border-indigo-200'
                ]"
              >
                ...
                <div class="flex items-center gap-2 text-sm text-gray-600">
                  <div class="w-full bg-gray-200 rounded-full h-2">
                    <div class="bg-indigo-600 h-2 rounded-full" :style="{ width: `${getProgress(subtopic)}%` }" />
                  </div>
            ...
            <div class="p-4 border-b">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  ...
                  <ChevronRight class="w-4 h-4 text-gray-400" />
                  <span class="text-sm font-medium text-gray-900">{{ selectedSubtopic?.title }}</span>
                </div>
                <div class="flex items-center gap-2 text-sm text-gray-600">
                  <span>Question {{ currentQuestionIndex + 1 }} of {{ selectedSubtopic?.questions.length }}</span>
                </div>
              </div>
            </div>
            <div class="p-6">
              ...
              <div class="space-y-3">
                <button
                  ...
                  :class="[
                    ...
                    answers[currentQuestion.id] === index ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 hover:border-indigo-200'
                  ]"
                >
                  <Circle v-if="answers[currentQuestion.id] !== index" class="w-5 h-5 text-gray-400" />
                  <CheckCircle2 v-if="answers[currentQuestion.id] === index" class="w-5 h-5 text-indigo-600" />
                  <span>{{ option }}</span>
                </button>
              </div>
              ...
              <div class="mt-6 flex gap-4">
                <button
                  ...
                  :class="[
                    ...
                    showFacts ? 'bg-indigo-50 border-indigo-500 text-indigo-700' : 'border-gray-300 text-gray-700 hover:bg-gray-50'
                  ]"
                >
                  <Info class="w-4 h-4" />
                ...
                <button
                  ...
                  :class="[
                    'flex items-center gap-2 px-4 py-2 rounded-md border',
                    showExamples ? 'bg-indigo-50 border-indigo-500 text-indigo-700' : 'border-gray-300 text-gray-700 hover:bg-gray-50'
                  ]"
                >
                  <Code class="w-4 h-4" />
              ...
              <!-- Facts Section -->
              <div v-if="showFacts && currentQuestion.facts" class="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                <h4 class="text-lg font-medium text-blue-800 flex items-center gap-2 mb-3">
                  <Lightbulb class="w-5 h-5" />
                  Key Facts
                </h4>
                <ul class="space-y-2">
                  <li v-for="(fact, index) in currentQuestion.facts" :key="index" class="flex items-start gap-2">
              ...
              <!-- Examples Section -->
              <div v-if="showExamples && currentQuestion.examples" class="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <div class="space-y-2">
                  <div
                    v-for="(example, index) in currentQuestion.examples" :key="index"
              ...
              <div class="flex justify-between items-center">
                <button
                  ...
                  :class="[
                    ...
                    currentQuestionIndex === 0 ? 'text-gray-400 cursor-not-allowed' : 'text-indigo-600 hover:bg-indigo-50'
                  ]"
                >
                ...
                <div class="flex gap-2">
                  <button
                    v-for="(question, index) in selectedSubtopic?.questions" :key="index"
                    @click="setCurrentQuestionIndex(index)"
                    :class="[
                      'w-8 h-8 rounded-full flex items-center justify-center',
                      index === currentQuestionIndex ? 'bg-indigo-600 text-white' : answers[selectedSubtopic.questions[index].id] !== undefined ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-200 text-gray-600'
                    ]"
                  >
                    {{ index + 1 }}
                  </button>
                ...
                <button
                  ...
                  :class="[
                    'flex items-center gap-2 px-4 py-2 rounded-md',
                    currentQuestionIndex === (selectedSubtopic?.questions.length ?? 0) - 1 ? 'text-gray-400 cursor-not-allowed' : 'text-indigo-600 hover:bg-indigo-50'
                  ]"
                >
                  Next
    ...
    <!-- Import Modal -->
    <div v-if="isImportModalOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4">
        ...
        <div class="p-6">
          <div
            ...
            @dragenter="handleDrag" @dragleave="handleDrag" @dragover="handleDrag" @drop="handleDrop"
          >
            <input
              ref="fileInputRef"
              type="file"
...
<script setup>
  import { BookOpen, ChevronRight, CheckCircle2, Circle, Search, ChevronDown, ChevronUp, ArrowLeft, ArrowRight, Upload, X, Info, Code, Lightbulb } from 'lucide-vue-next';
  ...
  const currentQuestion = computed(() => selectedSubtopic.value?.questions[currentQuestionIndex.value] || null);
  ...
  const handleNextQuestion = () => {
    if (selectedSubtopic.value && currentQuestionIndex.value < selectedSubtopic.value.questions.length - 1) {
